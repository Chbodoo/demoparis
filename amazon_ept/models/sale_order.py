...
class sale_order(models.Model):
    ...
    def action_confirm(self):
        ...
        for picking in self.picking_ids:
            picking.seller_id = self.seller_id and self.seller_id.id or False
            picking.amazon_instance_id=self.amz_instance_id and self.amz_instance_id.id or False
        return result
            ...
            for order in orders:
                ...
                for order_line_wrapper_obj in list_of_orderlines_wrapper:
                    ...
                    if not skip_order:
                        ...
                        if amazon_order:
                            auto_work_flow_obj.auto_workflow_process(instance.auto_workflow_id.id,[amazon_order.id])
...
#             else:
            state_id=result_state and result_state.id
    ...
    def get_qty_for_phantom_type_products(self,order,picking,order_ref,carrier_name,shipping_level_category,message_id,fulfillment_date_concat):
        ...
        for sale_line_id,product_ids in phantom_product_dict.items():
            ...
            if not moves:
                ...
                amazon_order_line=self.move[0].sale_line_id#get_amazon_sale_line(moves[0])
                amazon_order_item_id=self.move[0].sale_line_id.amazon_order_item_id
        ...
        for amazon_order in amazon_orders:
            for picking in amazon_order.picking_ids:
                ...
                if not shipping_level_category:
                    continue
                if not amazon_order.amz_instance_id.manage_multi_tracking_number_in_delivery_order:
                    tracking_no=picking.carrier_tracking_ref
                    parcel.update({
                                    'tracking_no':tracking_no or '',
                                    'order_ref':order_ref,
