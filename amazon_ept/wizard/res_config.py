...
class amazon_config_settings(models.TransientModel):
    ...
    company_for_amazon_id = fields.Many2one('res.company',string='Company')
    country_id = fields.Many2one('res.country',string = "Country")
    ...
    order_update_interval_type = fields.Selection( [('minutes', 'Minutes'),
            ('hours','Hours'),  ('days', 'Days'),('weeks', 'Weeks'), ('months', 'Months')], 'Update Order Interval Unit')    
    ...
    order_auto_import_xml_or_flat=fields.Boolean(string='Auto Import FBM Order?')#import  order  By report
    ...
    buynow_manage_customer_returns = fields.Boolean(string="Buy now")
...
#         if ctx.get('params',{}).get('model','')=='res.config.settings':
...
#                 seller_id = config_rec.seller_id.id 
    ...
    def onchange_instance_id(self):
        ...
        if instance:            
            ...
            values['producturl_prefix']= instance.producturl_prefix
    ...
    def execute(self):
        ...
        if instance:
            ...
            values['producturl_prefix']= self.producturl_prefix
